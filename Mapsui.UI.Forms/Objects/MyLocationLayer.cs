...
namespace Mapsui.UI.Objects
{
    ...
    public class MyLocationLayer : MemoryLayer
    {
        ...
        private static int deviceMovingId = -1;
        private static int deviceStillId = -1;

        int MovingId => IsDevice ? deviceMovingId : bitmapMovingId;
        int StillId => IsDevice ? deviceStillId : bitmapStillId;
        ...
        public bool IsMoving
        {
            ...
            set
            {
                ...
                {
                    isMoving = value;
                    ((SymbolStyle)feature.Styles.First()).BitmapId = isMoving ? MovingId : StillId;
                }
            }
        }
        ...
        public bool IsDevice
        {
            ...
            set
            {
                if (isDevice != value)
                {
                    ...
                    if (isDevice)
                    {
                        ((SymbolStyle)feature.Styles.First()).Opacity = 1;
                    }
                    else
                    {
                        ((SymbolStyle)feature.Styles.First()).Opacity = showMarker ? 1 : 0;
                    }
                }
            }
        }

        // sometimes we don't really want to show the marker.
        ...
        public MyLocationLayer(IMapView view)
        {
            ...
            if (deviceMovingId == -1)
            {
                var bitmapMoving = Mapsui.Utilities.EmbeddedResourceLoader.Load($"Images.DeviceMoving.svg", typeof(MyLocationLayer));

                if (bitmapMoving != null)
                {
                    // Register bitmap
                    deviceMovingId = BitmapRegistry.Instance.Register(bitmapMoving);
                }
            }

            if (deviceStillId == -1)
            {
                var bitmapStill = Mapsui.Utilities.EmbeddedResourceLoader.Load($"Images.DeviceStill.svg", typeof(MyLocationLayer));

                if (bitmapStill != null)
                {
                    // Register bitmap
                    deviceStillId = BitmapRegistry.Instance.Register(bitmapStill);
                }
            }
        ...
        public void UpdateMyLocation(Position newLocation, bool isDevice = false, bool animated = true)
        {
            IsDevice = isDevice;
